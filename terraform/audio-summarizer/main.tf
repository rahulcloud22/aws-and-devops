...
#S3 CORS Configuration: Enables browsers from allowed origins (e.g., localhost:3000) 
# to directly upload/download files from S3 using presigned URLs. 
#Without it, the browser blocks cross-origin requests even if S3 accepts them due to the browser's Same-Origin Policy.
resource "aws_s3_bucket_cors_configuration" "uploads" {
  ...
  cors_rule {
    ...
    expose_headers  = ["ETag"] # response header that S3 returns as an identifier for an uploaded object. Makes the ETag header accessible from JavaScript: 
    max_age_seconds = 3000
  }
}

# Todo - Least privilege principle  - Restrict S3 bucket access to only CloudFront and Transcribe service.
    ...
    Statement = [
      {
        Effect = "Allow"
        Action = [
          "s3:GetObject", # for Transcription job
          "s3:PutObject"
        ]
        Resource = [
...
resource "aws_apigatewayv2_api" "http_api" {
  name          = "audio-api"
  protocol_type = "HTTP"
  cors_configuration { # CORS configuration allows the frontend (e.g., React app) to make cross-origin requests to the API Gateway endpoints without being blocked by the browser's Same-Origin Policy. It specifies which origins, methods, and headers are allowed for cross-origin requests.
    allow_origins = [
      "http://localhost:5174",
