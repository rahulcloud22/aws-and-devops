terraform {
  required_providers {
    aws = {
      ...
      version = "~> 6.0" #Use version 6.0 or newer, but not 7.0 or higher. [6.1, 6.5] etc
    }
  }
...
provider "aws" {
  alias   = "A_WEST"
  region  = "us-west-2" #west-1 has only two AZs a & c for new accounts. west-2 is preferred
  profile = "rahul-cloud"
}
